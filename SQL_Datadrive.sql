...
-- CONSULTA 1: Ver todos los bundles con su descuento y fuerza
-- El "lift" mayor a 1 significa que los productos se compran
-- juntos mas de lo esperado por azar
SELECT
...
-- CONSULTA 2: Cuantas veces se vendio cada kit
SELECT
...
GROUP BY b.nombre, b.tipo_vehiculo
ORDER BY Veces_Vendido DESC;
GO

-- CONSULTA 3: Pares de productos que mas se compran juntos
SELECT
    d1.codigo_producto  AS Producto_A,
    d2.codigo_producto  AS Producto_B,
    COUNT(*)            AS Veces_Juntos
FROM Detalle_Venta d1
INNER JOIN Detalle_Venta d2
    ON  d1.id_venta = d2.id_venta
    AND d1.codigo_producto < d2.codigo_producto
GROUP BY d1.codigo_producto, d2.codigo_producto
ORDER BY Veces_Juntos DESC;
GO

-- CONSULTA 4: Ventas por mes y canal
SELECT
    FORMAT(fecha_venta, 'yyyy-MM') AS Mes,
    canal                           AS Canal,
    COUNT(*)                        AS Num_Ventas,
    SUM(total)                      AS Ingresos_COP
FROM Ventas
GROUP BY FORMAT(fecha_venta, 'yyyy-MM'), canal
ORDER BY Mes DESC;
GO

-- CONSULTA 5: Productos con stock bajo (alerta de reposicion)
SELECT
    i.codigo_producto   AS Codigo,
    p.nombre            AS Producto,
    p.tipo_vehiculo     AS Vehiculo,
...
-- CONSULTA 6: Ticket promedio segun tipo de vehiculo del cliente
SELECT
    c.tipo_vehiculo     AS Tipo_Cliente,
    COUNT(v.id_venta)   AS Total_Compras,
    AVG(v.total)        AS Ticket_Promedio_COP,
...
-- CONSULTA 7: Estado de las facturas
SELECT
    estado              AS Estado_Factura,
    COUNT(*)            AS Cantidad,
...
-- CONSULTA 8: Margen de ganancia por producto
SELECT
    codigo              AS Codigo,
    nombre              AS Producto,
    tipo_vehiculo       AS Vehiculo,
    ...
    (precio_venta - precio_costo) AS Ganancia_Unitaria,
    ROUND(100.0*(precio_venta-precio_costo)/precio_venta, 1) AS [Margen %]
FROM Productos
ORDER BY [Margen %] DESC;
GO

-- CONSULTA 9: Top 10 clientes que mas gastan
SELECT TOP 10
    c.nombres + ' ' + c.apellidos  AS Cliente,
    c.ciudad                        AS Ciudad,
    c.tipo_vehiculo                 AS Vehiculo,
...
-- CONSULTA 10: Que productos tiene cada bundle
SELECT
    b.id_bundle     AS Codigo_Kit,
    b.nombre        AS Nombre_Kit,
    p.nombre        AS Producto_Incluido,
    p.precio_venta  AS Precio_Individual
FROM Bundles b
INNER JOIN Detalle_Bundle db2 ON b.id_bundle = db2.id_bundle
INNER JOIN Productos p ON db2.codigo_producto = p.codigo
ORDER BY b.id_bundle, p.nombre;
GO